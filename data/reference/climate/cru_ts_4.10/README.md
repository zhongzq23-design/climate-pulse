...
- `tmp` — annual mean temperature (°C), calendar-day-weighted from monthly means
- `pre` — annual precipitation total (mm/year), sum of monthly totals
- `vap` — annual mean actual vapour pressure (hPa), calendar-day-weighted from monthly means
- `vpd` — annual mean vapour pressure deficit (hPa), calculated at monthly resolution first and then calendar-day-weighted to annual mean
