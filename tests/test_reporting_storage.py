...
class ReportingStorageTests(unittest.TestCase):
    ...
    def test_geometry_semantics_separate_exposure_context_and_risk(self):
        ...
        self.assertEqual(
            geometry_semantics('Wildfire', 'GDACS fire polygon')['grade'],
            'context_grade',
        )
        self.assertEqual(geometry_semantics('Storm', 'GDACS wind polygon')['grade'], 'context_grade')
        self.assertEqual(
            geometry_semantics('Storm', 'GDACS observed wind 34 kt polygon')['grade'],
            'exposure_grade',
        )
