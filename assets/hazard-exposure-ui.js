...
(() => {
  ...
  detailCells = function (e) {
    ...
    if (e.type === 'Wildfire') {
      ...
      if (hasNum(x.population_wind_39kt)) rows.push(row('GDACS lower-threshold wind exposure', `${fmtPop(x.population_wind_39kt)} people`, `${sourceText(provenance(e, 'population_wind_39kt'))} · raw field pop39; see Methods for 34 kt documented threshold`, 'source'));
      if (hasNum(x.population_wind_74kt)) rows.push(row('GDACS higher-threshold wind exposure', `${fmtPop(x.population_wind_74kt)} people`, `${sourceText(provenance(e, 'population_wind_74kt'))} · raw field pop74; see Methods for 64 kt documented threshold`, 'source'));
      ...
      if (hasNum(x.population_ts_or_impact_footprint)) rows.push(row('Population in mapped TC context footprint', `${fmtPop(x.population_ts_or_impact_footprint)} people`, `${sourceText(provenance(e, 'population_ts_or_impact_footprint'), 'Climate Pulse · fallback')} · context/screening; not headline exposure`, 'fallback'));
  ...
  function exposureSummary(e) {
    ...
    if (e?.type === 'Storm') {
      const bits = [];
      if (hasNum(x.population_wind_39kt)) bits.push(`${fmtPop(x.population_wind_39kt)} people in the GDACS lower-threshold wind exposure field (raw pop39; documented method uses 34 kt)`);
      if (hasNum(x.population_wind_74kt)) bits.push(`${fmtPop(x.population_wind_74kt)} in the GDACS higher-threshold wind exposure field (raw pop74; documented method uses 64 kt)`);
      if (hasNum(x.population_storm_surge)) bits.push(`${fmtPop(x.population_storm_surge)} in the storm-surge zone`);
      if (!bits.length && hasNum(x.population_ts_or_impact_footprint)) bits.push(`${fmtPop(x.population_ts_or_impact_footprint)} people in a mapped TC context/screening footprint; not headline exposure`);
      const gt = gdpText(x);
      return `${bits.length ? `${bits.join('; ')}. Source exposure is modelled, not observed harm; mapped TC geometry must separately pass the strict headline gate.` : ''}${gt ? ` ${gt}` : ''}`.trim();
    }
    if (e?.type === 'Flood') {
      const bits = [];
