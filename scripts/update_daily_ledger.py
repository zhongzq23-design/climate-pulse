...
from enforce_population_exposure_policy import canonical_geometry_semantics
...
def geometry_semantics(event_type: str, footprint_method: Any, geometry_qc: Any = None) -> dict[str, Any]:
    """Use the same canonical fail-closed policy as the report-integrity gate."""
    return canonical_geometry_semantics(event_type, footprint_method, geometry_qc)
...
def reporting_geometry(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        return {
            ...
            "geometry_semantics": geometry_semantics(str(event.get("type") or ""), method, qc),
            "geometry_qc": qc,
