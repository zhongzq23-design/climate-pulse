...
polygon returned by the endpoint. Instead, prefer one source polygon feature that
is spatially aligned with the reported event coordinate and fail closed when no
credible center-aligned polygon exists.

The returned geometry remains a GDACS reported event/affected-area context layer.
This module never infers satellite-observed or modelled inundation extent and never
promotes the selected context polygon to exposure-grade.
"""
...
VERY_LARGE_CONTEXT_AREA_KM2 = 300_000.0
...
def _base_context_qc() -> dict[str, Any]:
    return {
        "semantic_role": "reported_event_area_context",
        "exposure_grade_eligible": False,
        "inundation_status": "not_established",
        "scientific_interpretation": (
            "Selected GDACS flood geometry is a reported event/affected-area context feature; "
            "it is not interpreted as observed or modelled inundation extent."
        ),
    }
...
def select_flood_geometry(features: list[dict[str, Any]], lon: float, lat: float):
    ...
    QC is intentionally fail-closed for source-feature selection. A source polygon
    farther than 75 km from the reported event coordinate is suppressed. A selected
    polygon remains context-grade regardless of spatial QC success.
    """
    cs = candidates(features, lon, lat)
    base = _base_context_qc()
    if not cs:
        return None, {
            **base,
            "status": "failed",
    ...
    qc = {
        **base,
        ...
        "selected_feature_metadata_context_like": bool(best.metadata_context_like),
    ...
    if best.area_km2 > VERY_LARGE_CONTEXT_AREA_KM2:
        qc["warning"] = "very_large_reported_event_area_context"
        qc["warning_interpretation"] = (
            "Large area is retained only as reported-event context and must not be read as flooded land area."
        )
